DROP FUNCTION IF EXISTS public.claim_generation_job(uuid);

CREATE OR REPLACE FUNCTION public.claim_generation_job(_job_id uuid)
 RETURNS TABLE(id uuid, user_id uuid, brief_hash text, brief jsonb, attempts integer, itinerary_id uuid)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  WITH claimed AS (
    ...
    RETURNING public.generation_jobs.id,
              ...
              public.generation_jobs.itinerary_id
  )
  SELECT * FROM claimed;
END;
$function$;
